...
class Inception(nn.Module):
    def __init__(self, in_planes, n1x1, n3x3red, n3x3, n5x5red, n5x5, pool_planes, useRRSVM=False):
        ...
        if self.useRRSVM:
            self.pool_r = RRSVM.RRSVM_Module(in_planes, kernel_size=3, stride=1, padding=1)
...
class GoogLeNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.b3(out)
        if self.useRRSVM:
            out = self.pool1(out) + self.pool1_r(out)
        ...
        out = self.e4(out)
        if self.useRRSVM:
            out = self.pool2(out) + self.pool2_r(out)
        else:
            out = self.pool2(out)
        out = self.a5(out)
        out = self.b5(out)
        if self.useRRSVM:
            out = self.pool3(out) + self.pool3_r(out)
        else:
            out = self.pool3(out)
