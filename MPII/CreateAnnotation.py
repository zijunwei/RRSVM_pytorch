...
# modified from https://github.com/rohitgirdhar/AttentionalPoolingAction/blob/master/utils/dataset_utils/gen_tfrecord_mpii.py
import math
import os
import random
import sys
...
rel_dataset_dir = 'datasets/MPII'
_MPII_MAT_FILE = os.path.join(user_root, rel_dataset_dir, 'mpii_human_pose_v1_u12_2/mpii_human_pose_v1_u12_1.mat')
_IMG_DIR = os.path.join(user_root, rel_dataset_dir, 'images')


dataset_dir = os.path.join(user_root, rel_dataset_dir)
_SPLITS_PATH = os.path.join(user_root, rel_dataset_dir, 'mpii_human_pose_v1_u12_2/images_mpii_{:s}.txt')
...
def main():
  ...
  for aid,annot in enumerate(annots):
    ...
    if os.path.exists(os.path.join(_IMG_DIR, imname)):
      full_annotations[this_split].append(image_obj)
  ...
  train_ids = range(len(full_annotations['train']))
  random.shuffle(train_ids)
  full_annotations['train'] = [full_annotations['train'][i] for i in train_ids]
  img_id_in_split['train'] = [img_id_in_split['train'][i] for i in train_ids]

  with open(os.path.join(dataset_dir, 'imnames.txt'), 'w') as fout:
    fout.write('\n'.join(all_imnames))

  output_file = 'annotation.pkl'
  fout = open(output_file, 'wb')
  pickle.dump(full_annotations, fout)
  fout.close()
  # test:
