...
#pragma omp parallel for private(elt)
    ...
    for (elt = 0; elt < batchSize; elt ++) {
        ...
        fill_output_3d(sorted_input_2d_data, sorted_index_2d_data, s_data, elt, kH,  kW,  nInputPlane,  outputHeight,  outputWidth,  output_data, indices_data, THCState_getCurrentStream(state));
...
void RRSVM_accGradParameters_cuda(THCudaTensor *input, THCudaLongTensor * indices, THCudaTensor *gradOutput, THCudaTensor *gradS,
    ...
    real *gradOutput_data, *gradS_data;
    long *indices_data;
