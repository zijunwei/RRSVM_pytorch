...
class RRSVM_F(torch.autograd.Function):
    ...
    def forward(self, input, t):
        ...
        assert self.kernel_size * self.kernel_size == t.size(1), 'Kernel size should be the same as s size'
        k_s = t.dim()
        if k_s != 2:
        ...
        output, indices = self._update_output(input, t)
        self.intermediate = (input, t, indices,)
...
class RRSVM_Module(torch.nn.Module):
    # comapred to RRSVM. this is monotonous
    ...
    def __init__(self, in_channels, kernel_size, stride=None, padding=0, dilation=1, p_constraint=False, return_indices=False):
        ...
        self.padding = padding
        self.dilation = dilation
        self.t = Parameter(torch.Tensor(in_channels, self.kernel_size * self.kernel_size))
        # initialize s:
        n_elt = self.kernel_size * self.kernel_size
        init_val = 2. / n_elt * (n_elt - 1)
        self.t.data.fill_(init_val)
