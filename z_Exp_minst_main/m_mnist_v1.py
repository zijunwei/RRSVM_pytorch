...
class Net(nn.Module):
    def __init__(self, pool_method):
        ...
        if pool_method == 'max':
            ...
            self.pool1 = RRSVM_Module(in_channels=d1, init='eps_max', kernel_size=gksize, stride=2, padding=gpsize)
            self.pool2 = RRSVM_Module(in_channels=d2, init='eps_max', kernel_size=gksize, stride=2, padding=gpsize)
        ...
        elif pool_method == 'R1+M':
            self.pool1 = torch.nn.MaxPool2d(kernel_size=ksize, stride=2, padding=psize)
            self.pool2 = torch.nn.MaxPool2d(kernel_size=ksize, stride=2, padding=psize)
            self.gpool1 = RRSVM_Module(in_channels=d1, init='eps_max', kernel_size=gksize, stride=2, padding=gpsize)
        elif pool_method == 'R2+M':
            self.pool1 = torch.nn.MaxPool2d(kernel_size=ksize, stride=2, padding=psize)
            self.pool2 = torch.nn.MaxPool2d(kernel_size=ksize, stride=2, padding=psize)
            # self.gpool1 = RRSVM_Module(in_channels=d1, init='eps_max', kernel_size=gksize, stride=2, padding=gpsize)
            self.gpool2 = RRSVM_Module(in_channels=d2, init='eps_max', kernel_size=gksize, stride=2, padding=gpsize)
    ...
    def forward(self, x):
        # should be after relu
        # x = F.relu(F.max_pool2d(self.conv1(x), 2))
        x = self.conv1(x)
        x = F.relu(x)

        if self.pool_method in {'R+M', 'A+M', "M+M", 'GR+M', 'R1+M'}:
            x1 = self.pool1(x)
            x2 = self.gpool1(x)
            # x = torch.cat((x1, x2), 1)
            x = x1 + x2
        ...
        if self.pool_method in {'R+M', 'A+M', "M+M", 'GR+M', 'R2+M'}:
            x1 = self.pool2(x)
            x2 = self.gpool2(x)
            # x = torch.cat((x1, x2), 1)
            x = x1 + x2
...
def train(epoch):
    ...
    optimizer = optim.SGD(model.parameters(), lr=learning_rate(args.lr, epoch), momentum=args.momentum)
    model.train()
...
for epoch in range(1, args.epochs + 1):
    ...
    s_train_acc, s_train_loss = test(train_loader)
    print("Testing on test data")
    s_test_acc, s_test_loss = test(test_loader)
    train_acc.append(s_train_acc)
    train_loss.append(s_train_loss)
    test_acc.append(s_test_acc)
    test_loss.append(s_test_loss)

plt.subplot(2, 1, 1)
plt.plot(range(args.epochs), train_acc, 'r')
plt.plot(range(args.epochs), test_acc, 'g')
plt.title('{:s} -- Acc, R: train, G:test'.format(args.pool_method))

plt.subplot(2, 1, 2)
plt.plot(range(args.epochs), train_loss, 'r')
plt.plot(range(args.epochs), test_loss, 'g')
plt.title('{:s} -- Loss, R: train, G: test'.format(args.pool_method))
